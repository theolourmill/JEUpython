...
#pour pouvoir parcourir les machines dans une boucle ou pour éviter la redondance
suffixes = ["", "lv2", "lv3", "lv4", "lv5"]

#pour le futur système de stat
def compteur_temps():
    game.compteur_temps_de_jeu()
    fenetre_jeu.after(1000, compteur_temps)

def actualiser_affichage_ressource():
    ...
    fenetre_jeu.after(100, actualiser_affichage_ressource)

def actualisation_ressource_click():
    game.ajouter_ressource()
    
def actualiser_prod_machine_unique():
    game.production_machine()
    fenetre_jeu.after(game.config.dict_ressource['tick-rate'], actualiser_prod_machine_unique)
...
bouton_ressource = tk.Button(fenetre_jeu, text="PRODUIRE (+1)", command=actualisation_ressource_click, bg="lightgreen", width=30, height=2)
...
actualiser_prod_machine_unique()
compteur_temps()
actualiser_affichage_ressource()
